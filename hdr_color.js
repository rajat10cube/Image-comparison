const lin2pq = ( L )=>{
    ...
    const Lmax = 10000
    //Lmin = 0.005
    const n    = 0.15930175781250000
    const m    = 78.843750000000000
    const c1   = 0.83593750000000000
    const c2   = 18.851562500000000
    const c3   = 18.687500000000000
    let im_t = new Array(L.length).fill(0).map(() => new Array(L[0].length).fill(0));
...
const __rgb2020_2xyz = [    [0.6370, 0.1446, 0.1689],
                            [0.2627, 0.6780, 0.0593],
                            [0.0000, 0.0281, 1.0610] ];

const __xyz2rgb2020 =  [    [ 1.716502508360628, -0.355584689096764,  -0.253375213570850],
...
export const im_ctrans = ( im, fromCS=null, toCS=null, M=null, exposure=1 )=>{
    ...
    */
    ...
    if(M == null){
        M = math.multiply(xyz2out,in2xyz);
    }
    let transposeM = math.transpose(M)
    let col_vec_out = math.multiply(col_vec, math.multiply(transposeM,exposure));
    console.log("col_vec_out",col_vec_out);
    if(to_nl_func!=null){ // Non-linearity, if needed
